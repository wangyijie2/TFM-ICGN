...
def result_is_success(result, parent=None):
    keys = ('u', 'v', 'ux', 'uy', 'vx', 'vy', 'uxx', 'uxy', 'uyy', 'vxx', 'vxy', 'vyy', 'zncc', 'convergence', 'iteration')
    if not all(np.isfinite(result[k]) for k in keys):
        return False
    if not (result['zncc'] >= 0.7 and 0 <= result['convergence'] < CONV_CRIT and 1 <= result['iteration'] <= MAX_ITER):
        return False
    if parent is None:
        return True
    return 2.0 * (1.0 - result['zncc']) < 2.0 and abs(result['u'] - parent['u']) < STEP_X and abs(result['v'] - parent['v']) < STEP_X
...
def main():
    ...
    def solve_point(px, py, u0, v0, gradients=None, parent=None):
        nonlocal processed, successes, fails, total_iter, solver_calls
        nonlocal succ_time_total, propagation_blocked
        processed += 1
        status[py, px] = 1
        ...
        if result_is_success(result, parent):
            ...
            heapq.heappush(queue, (2.0 * (1.0 - result['zncc']), next(sequence), px, py, result))
        else:
            fails += 1
            propagation_blocked += 1
    ...
    while queue:
        ...
        for (dx, dy) in neighbors4:
            ...
            solve_point(px, py, u0, v0, gradients, parent=parent)
    ...
    np.savetxt(os.path.join(DATA_DIR, 'RG_ICGN2_valid.csv'), (status[MARGIN:H - MARGIN, MARGIN:W - MARGIN] == 0).astype(np.uint8), delimiter=',', fmt='%d')
